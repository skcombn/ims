...
import { useSuppliersActive } from '../react-query/suppliers/useSuppliersActive';
...
const SuppliersListingReport = () => {
  const isFetching = useIsFetching();
  const navigate = useNavigate();
  const { colorScheme } = useMantineTheme();
  const field_width = '150';
  const field_gap = '3';
  const { suppliersactive } = useSuppliersActive();
  const [rptbatch, setRptBatch] = useRecoilState(RptState);
  const [editrptidstate, setEditRptIdState] = useRecoilState(editRptIdState);
  const [rowSelection, setRowSelection] = useState({});

  const batch = suppliersactive;
  ...
  const columns = useMemo(
    () => [
      ...
      {
        header: 'Tel',
        accessorFn: row => row.s_tel1 && ' / ' && row.s_tel2,
        size: 100,
        mantineTableBodyCellProps: {
          align: 'left',
        },
  ...
  const mantinetable = useMantineReactTable({
    ...
    state: { rowSelection },
    getRowId: originalRow => originalRow.s_suppno,
    enableTopToolbar: true,
    initialState: { sorting: [{ id: 's_supp', desc: false }] },
  ...
  const handlePrintListing = () => {
    ...
    if (count === 0) {
      Toast({
        title: `No suppliers being selected!`,
        status: 'warning',
      });
    } else {
      const batchdata = batch.filter(r =>
        Object.keys(rowSelection).find(key => key === r.s_suppno)
      );
      ...
      navigate('/supplierslistingrptprint');
    }
  };
  ...
  const handleExportExcel = rows => {
    ...
    const rowData = rows.map(row => [
      row.original.s_suppno,
      row.original.s_supp,
      row.original.s_tel1 && ' / ' && row.original.s_tel2,
      row.original.s_email,
      row.original.s_fax,
      row.original.s_contact,
    ]);
    const colWidths = [
      { wch: 15 },
      { wch: 30 },
      ...
      >
        ...
        >
          <GridItem colSpan={2} mt={19}>
            <Center>
              <Heading size="md">Suppliers Listing Report</Heading>
            </Center>
          </GridItem>
          <GridItem colSpan={6}>
...
export default SuppliersListingReport;
