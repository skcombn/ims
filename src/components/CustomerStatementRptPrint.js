...
export const InvoiceHeader = ({ comp, add1, add2, add3 }) => (
...
export const BillTo = ({ doc, pageno, totalpage, statementdate }) => (
  <View style={headerstyles.colContainer}>
    ...
    <View style={headerstyles.rowContainer}>
      <Text style={headerstyles.billtolabel}>{doc.c_cust}</Text>
      <Text style={headerstyles.doclabel}>Cust No:</Text>
      <Text style={headerstyles.docfield}>{doc.c_custno}</Text>
    </View>
    <View style={headerstyles.rowContainer}>
      <Text style={headerstyles.billtofield}>{doc.c_add1}</Text>
    ...
    <View style={headerstyles.rowContainer}>
      <Text style={headerstyles.billtofield}>{doc.c_add2}</Text>
      <Text style={headerstyles.doclabel}>Area:</Text>
      <Text style={headerstyles.docfield}>{doc.c_area}</Text>
    </View>
    <View style={headerstyles.rowContainer}>
      <Text style={headerstyles.billtofield}>{doc.c_add3}</Text>
    ...
    <View style={headerstyles.rowContainer}>
      <Text style={headerstyles.billtofield}>{doc.c_tel}</Text>
      {/* <Text style={headerstyles.doclabel}>Page No:</Text>
      <Text style={headerstyles.docfield}>
        {pageno} / {totalpage}
      </Text> */}
...
export const InvoiceTableRow = ({ items, totals }) => {
...
const InvoiceItemsTable = ({ items, lastpage, totals }) => (
...
const CustomerStatementPrint = () => {
  ...
  const [batchdetls, setBatchdetls] = useRecoilState(RptDetlsState);
  const [editaridstate, setARIdState] = useRecoilState(editRptIdState);
  const { setup } = useSetup();
  const detlsdata = _.chunk(batchdetls, tableRowsCount);

  console.log('print batch', batch);
  console.log('print batchdetls', batchdetls);
  ...
  const MyDocument = () => (
    <Document>
      {batch.map(rec => {
        const totals = {
          lastbfbal: rec.c_bfbal,
          balcurr: rec.c_balcur,
          bal30: rec.c_bal30,
          bal60: rec.c_bal60,
          bal90: rec.c_bal90,
          totbal: rec.c_totbal,
        ...
        const detls = batchdetls.filter(dr => dr.ar_custno === rec.c_custno);
        const detlsdata = _.chunk(detls, tableRowsCount);
        return detls.length > 0 ? (
            ...
            return (
              <Page size="A4" style={docstyles.page} key={index}>
                ...
                <BillTo
                  ...
                  statementdate={editaridstate.todate}
                />
          ...
          <Page size="A4" style={docstyles.page}>
            ...
            />
            <InvoiceTitle title="Statement Of Account" />
            <BillTo doc={rec} pageno={1} totalpage={1} />
