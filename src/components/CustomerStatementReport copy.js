...
import {
  arStatementRptState,
  arStatementDetlsRptState,
  editARStatementRptIdState,
} from '../data/atomdata';
...
import { InvoiceHeader, InvoiceTitle, BillTo } from './CustomerStatementPrint';
...
const CustomerStatementReport = () => {
  ...
  const [rptbatch, setRptBatch] = useRecoilState(arStatementRptState);
  const [rptbatchdetls, setRptBatchdetls] = useRecoilState(
    arStatementDetlsRptState
  );
  const [editarrptidstate, setARRptIdState] = useRecoilState(
    editARStatementRptIdState
  );
  ...
  const handleCalc = () => {
    ...
    customersactive
      ...
      .forEach((item, index) => {
        ...
        //setARMthViewCustno(prev => (prev = custno));
        //setARPeriod(prev => (prev = custno));
        ...
        balcur = armthview
          .filter(r => r.ar_custno === custno && r.ar_yearmonth === mthcur)
          .reduce((acc, rec) => {
        ...
        balrept = armthview
          .filter(r => r.ar_yearmonth > mth90 && r.ar_doctype === 'Receipt')
          .reduce((acc, rec) => {
            return acc + rec.ar_totalamt;
          }, 0);
        ...
        bal90 = Math.round(bal90 + bfbal, 2);
        if (balcur < 0) {
          bal30 = bal30 + balcur;
          balcur = 0;
        }
        ...
        totbal = balcur + bal30 + bal60 + bal90;
        ...
        const newRecDetls = receivablebymth.filter(item => {
          return (
            ...
            item.ar_date >= moment(fromdate).format('YYYY-MM-DD') &&
  ...
  const handleStatement = () => {
    ...
    if (count === 0) {
      Toast({
        title: `No employee being selected!`,
        status: 'warning',
      });
      return null;
    }
