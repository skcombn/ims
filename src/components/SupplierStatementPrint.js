...
const BillTo = ({ doc, pageno, totalpage, statementdate }) => (
  <View style={headerstyles.colContainer}>
    ...
    <View style={headerstyles.rowContainer}>
      <Text style={headerstyles.billtolabel}>{doc.ap_supplier}</Text>
      <Text style={headerstyles.doclabel}>Supp No:</Text>
      <Text style={headerstyles.docfield}>{doc.ap_suppno}</Text>
    </View>
    <View style={headerstyles.rowContainer}>
      <Text style={headerstyles.billtofield}>{doc.ap_add1}</Text>
    ...
    <View style={headerstyles.rowContainer}>
      <Text style={headerstyles.billtofield}>{doc.ap_add2}</Text>
      <Text style={headerstyles.doclabel}>Area:</Text>
      <Text style={headerstyles.docfield}>{doc.ap_area}</Text>
    </View>
    <View style={headerstyles.rowContainer}>
      <Text style={headerstyles.billtofield}>{doc.ap_add3}</Text>
    ...
    <View style={headerstyles.rowContainer}>
      <Text style={headerstyles.billtofield}>{doc.ap_tel}</Text>
      {/* <Text style={headerstyles.doclabel}>Page No:</Text>
      <Text style={headerstyles.docfield}>
        {pageno} / {totalpage}
      </Text> */}
...
const InvoiceTableRow = ({ items, totals }) => {
  const { lastbfbal } = totals;
  let sum = lastbfbal;
  let runningbal = [];
  items.forEach(rec => {
    if (rec.ap_doctype !== 'Receipt' && rec.ap_doctype !== 'Credit') {
      sum = sum + rec.ap_nettotal;
    } else {
      sum = sum - rec.ap_nettotal;
    }
    runningbal.push(sum);
  ...
  const rows = items.map((item, index) => (
    <View style={tablerowstyles.row} key={index}>
      ...
      <Text style={tablerowstyles.date}>{item.ap_podate}</Text>
      <Text style={tablerowstyles.type}>{item.ap_doctype}</Text>
      <Text style={tablerowstyles.debit}>
        {item.ap_doctype !== 'Receipt' && item.ap_doctype !== 'Credit'
          ? currency(item.ap_nettotal).format()
          : null}
      </Text>
      <Text style={tablerowstyles.credit}>
        {item.ap_doctype === 'Receipt' || item.ap_doctype === 'Credit'
          ? currency(item.ap_netttotal).format()
          : null}
      </Text>
...
const InvoiceTableFooter = ({ totals }) => {
  /*  const total = items
...
const InvoiceItemsTable = ({ items, total, lastpage, totals }) => (
...
const InvoiceBlankItemsTable = ({ items, total, lastpage, totals }) => (
  <View style={itemsstyles.tableContainer}>
    <InvoiceTableHeader />
    <InvoiceTableRow items={items} totals={totals} />
    <InvoiceTableBlankSpace rowsCount={tableRowsCount - items.length} />
...
const SupplierStatementPrint = () => {
  ...
  console.log('ap print totals', editapidstate);
  ...
  return (
    ...
    >
      ...
      >
        <HStack py={2} ml={10} spacing="3">
          <Grid templateColumns={'repeat(4,1fr)'} columnGap={3}>
            <GridItem colSpan={4}>
              <Button
                pl={10}
          ...
          <Document>
            ...
            {batchdetls.length > 1 ? (
                ...
                return (
                  <Page size="A4" style={docstyles.page} key={index}>
                    ...
                    <BillTo
                      ...
                      pageno={index + 1}
                      totalpage={detlsdata.length}
                      statementdate={editapidstate.todate}
                    />
                    ...
                    {detlsdata.length - 1 === index ? (
                      <AgingTable totals={editapidstate.totals} />
                    ) : null}
              ...
              <Page size="A4" style={docstyles.page}>
                ...
                />
                <InvoiceTitle title="Statement Of Account" />
                <BillTo doc={batch} pageno={1} totalpage={detlsdata.length} />

                <InvoiceBlankItemsTable
                  items={[]}
                  total={batch.sls_total}
                  // lastpage={detlsdata.length - 1 === index}
                  totals={editapidstate.totals}
...
export default SupplierStatementPrint;
